...
- Al integrar el PR de los eventos Monopoly, `ConfigManager.java` llegó con el `}` de cierre del accessor `monopoly()` metido dentro del siguiente método (`debt()`) por un rebase mal resuelto - error de compilación literal, corregido. `SubscriptionManagerTest.java` traía dos pruebas duplicadas (`inviteDoesNotChargeUntilAccepted`, `acceptInviteLeavesTheInvitePendingWhenThePayerCantAfford`): una primera copia rota que usaba una variable nunca declarada, seguida de la copia buena. Se borró la copia rota - de paso se perdió el único caso que probaba el cobro de la renovación tras aceptar una invitación (el camino directo `subscribe()` sigue cubierto por `processDueChargesAppliesTheTaxOnRenewalsToo`); si hace falta esa cobertura de nuevo, hay que reescribirla desde cero, el original no era recuperable.

### Nuevo: eventos económicos "Monopoly"
- Cada `eventsPerDay` días de juego (1 por defecto) el servidor sortea un evento ponderado de `monopoly.json` y lo anuncia por chat: multiplicador temporal de salario o de recompensa de misiones, un "mob buscado" que reparte un bounty extra entre quien lo dañó (con cupo opcional de muertes pagadas), un efecto de poción instantáneo para todos los conectados (`WINDFALL`, incluye debuffs si se configuran), o cara o cruz contra La Casa.
- `/monopoly coinflip <cantidad>` apuesta contra el sistema (paga comisión, quemada); `/monopoly coinflip <cantidad> <jugador>` reta a otro jugador (invitación pendiente, ambos pagan la comisión si se acepta). `/monopoly status` muestra el evento activo; `/monopoly roll [id]`/`/monopoly end` (OP 2) son herramientas de admin.
- Ficha completa en [`docs/features/monopoly.md`](docs/features/monopoly.md).
