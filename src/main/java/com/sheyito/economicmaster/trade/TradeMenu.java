 ...
 * Both players in a trade get their own {@code TradeMenu} instance, but the item-offer rows
 * point at the *same* {@link SimpleContainer} objects (just swapped, "mine" vs "theirs") held by
 * the shared {@link TradeSession} - vanilla's per-tick {@code broadcastChanges()} then keeps
 * both clients in sync with zero custom networking. Money is fixed at invite time (see
 * {@link TradeSession#moneyPledged()}) and only shown here, never edited from the GUI.
 */
public class TradeMenu extends AbstractContainerMenu {
    ...
    public TradeMenu(int containerId, Inventory playerInventory, TradeSession session, UUID viewerUuid) {
        ...
        // Row 3 (slots 27-35): fixed money offer, the optional message, filler, confirm, cancel.
        addSlot(new MirrorSlot(session.moneyDisplayContainer(), 0, 8, 72));
        addSlot(new MirrorSlot(session.messageDisplayContainer(), 0, 8 + 18, 72));
        for (int col = 2; col < 7; col++) {
            addSlot(new MirrorSlot(fillerContainer(Items.GRAY_STAINED_GLASS_PANE, " "), 0, 8 + col * 18, 72));
        }
    ...
    private static SimpleContainer fillerContainer(Item paneItem, String name) {
    ...
    public void clicked(int slotId, int button, ClickType clickType, Player player) {
        ...
        // Mirror rows, progress bar, money/message display and decorative filler: never interactive.
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (index < OFFER_SIZE) {
            ...
            if (session.isLocked() || !moveItemStackTo(original, 0, OFFER_SIZE, false)) {
                return ItemStack.EMPTY;
            }
        } else {
            return ItemStack.EMPTY;
